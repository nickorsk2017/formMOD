...
export const Fileinput = (props: FileinputProps) => {
    ...
    const renderFiles = () => {
      return files.map((fileInputValue: FileInputValue, index: number) => {
        ...
        return <div key={index} className={styles.fileContainer}>{fileInputValue.file.name}<button onClick={() => onDelete(fileInputValue)} className={styles.clearFile}>x</button></div>
      });
    }
